...
class RelativeViewObservaton(AbstractObservation):
    ...
    def get_mask_coordinates(self, agent):
        ...
        self.global_x_min = abs(min(0, pos_x - self.visible_radius))
        self.global_x_max = self.visible_range - max(
            0, pos_x + self.visible_radius - (self.world.map.SIZE_X - 1)
        )
        self.global_y_min = abs(min(0, pos_y - self.visible_radius))
        self.global_y_max = self.visible_range - max(
            0, pos_y + self.visible_radius - (self.world.map.SIZE_Y - 1)
        )
        self.obs_x_min = max(0, pos_x - self.visible_radius)
        self.obs_x_max = min(self.world.map.SIZE_X - 1, pos_x + self.visible_radius)
