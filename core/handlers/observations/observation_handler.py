...
class ObservationHandler:
    def __init__(self, config: DictConfig, world: AbstractWorld):
        ...
        self.observation_agent = generate_observation_agent(
            config=config, world=world
        )
    ...
    def observation_ind(self, agents: List[Agent], agent: Agent, agent_id: int) -> torch.Tensor:
        ...
        obs_agent = self.observation_agent.fill(agents, agent, agent_id, area_mask, coordinates)

        # input objects within sight
        obs_object = self.observation_object.fill(agents, agent, agent_id, area_mask, coordinates)

        obs_area = self.observation_area_fill(agents, agent, agent_id, area_mask, coordinates)
