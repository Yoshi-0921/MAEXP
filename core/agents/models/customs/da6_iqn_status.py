...
class DA6_IQN_Status(DA3_IQN):
    def __init__(self, config: DictConfig, input_shape: List[int], output_size: int):
        ...
        self.status_saliency_vector = nn.Parameter(torch.zeros(1, 1, config.model.embed_dim // 2))
        ...
        self.status_pos_embed = nn.Parameter(
            torch.zeros(
                1,
                3 * 3 + 1,
                config.model.embed_dim // 2
        ...
        self.status_blocks = nn.ModuleList(
            [
                block(
                    dim=config.model.embed_dim // 2,
                    num_heads=config.model.num_heads,
    ...
    def get_saliency_vector(self, state, output_attns: bool = False):
        local_x, relative_x, status_x = self.state_encoder(state)
        ...
        out = self.status_embed(status_x)
        status_saliency_vector = self.status_saliency_vector.expand(out.shape[0], -1, -1)
        out = torch.cat((status_saliency_vector, out), dim=1)
        out = out + self.status_pos_embed

        status_attns: List[npt.NDArray[np.float32]] = list()
        for blk in self.status_blocks:
            out, attn = blk.forward_attn(out)
            status_attns.append(attn.detach())

        out = self.cond_norm(out)
        status_saliency_vector = out[:, 0]

        saliency_vector = torch.cat([relative_saliency_vector, status_saliency_vector], dim=1)
        ...
        if output_attns:
            return saliency_vector, [local_attns, relative_attns, status_attns]
    ...
    def state_encoder(self, state):
        ...
        status_x = state["status_remains"].unsqueeze(1).view(-1, 1, 3, 3)

        return local_x, relative_x, status_x
